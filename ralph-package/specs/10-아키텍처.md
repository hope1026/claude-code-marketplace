...
4. Ralph가 선택된 adapter로 에이전트를 fresh context에서 한 번 실행한다.
5. Ralph가 `result.json`을 읽고 validation을 수행한다.
6. Ralph가 task 완료, retry, follow-up task 생성, blocked 전환 중 하나를 결정한다.
7. 모든 phase gate가 통과하면 최종 summary를 생성하고 job을 완료한다.
